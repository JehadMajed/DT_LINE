...
Add `archive/deploy/bootstrap.sh` — fresh Pi OS to fully working, idempotent,
in one command:
...
Commit the two missing units so the camera path is reproducible:
`archive/deploy/go2rtc.service`, `archive/deploy/tailscale-funnel.service`.
